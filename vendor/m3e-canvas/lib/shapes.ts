 ...
 *
 * SVG path data comes from the official Material Design shape assets and the
 * morph/rotation model mirrors material-components-android's
 * LoadingIndicatorAnimatorDelegate (Apache-2.0, see NOTICE).
...
type ShapeDef = { viewBox: number; d: string };

const SVG_DEFS: (ShapeDef | null)[] = [
  {
    viewBox: 144,
...
function svgPathToPoints(d: string): Point[] {
  const pts: Point[] = [];
  let cx = 0;
  let cy = 0;
  for (const { cmd, args } of parseSVGPath(d)) {
    if (cmd === "M") {
      ...
      for (let i = 0; i < args.length; i += 6) {
        ...
        pts.push(
          ...sampleCubic([cx, cy], [args[i], args[i + 1]], [args[i + 2], args[i + 3]], p3, SAMPLES_PER_CURVE),
        );
        cx = p3[0];
        cy = p3[1];
      }
...
function normalize(pts: Point[], viewBox: number): Point[] {
  const h = viewBox / 2;
  const c: Point[] = pts.map(([x, y]) => [(x - h) / h, (y - h) / h]);
  let x0 = Infinity;
  let x1 = -Infinity;
  let y0 = Infinity;
  let y1 = -Infinity;
...
function generateOval(n: number): Point[] {
  const pts: Point[] = [];
  for (let i = 0; i < n; i++) {
    const t = (i / n) * 2 * Math.PI;
    pts.push([Math.cos(t), 0.74 * Math.sin(t)]);
  }
  return pts;
}
...
export function getShapes(): Point[][] {
  ...
  cache = SVG_DEFS.map((def) =>
    def === null
      ? generateOval(POINTS_PER_SHAPE)
      : resample(normalize(svgPathToPoints(def.d), def.viewBox), POINTS_PER_SHAPE),
  );
  return cache;
}

export function morphedShape(fraction: number): Point[] {
  const shapes = getShapes();
  const idx = Math.floor(fraction);
  const from = ((idx % SHAPE_COUNT) + SHAPE_COUNT) % SHAPE_COUNT;
  const to = (from + 1) % SHAPE_COUNT;
  const t = Math.max(0, Math.min(1, fraction - idx));
  const a = shapes[from];
  const b = shapes[to];
  const out: Point[] = new Array(a.length);
...
/** Spring matching Android's SpringForce (mass = 1). */
export class Spring {
  private k: number;
  private c: number;
  pos = 0;
...
/** Mirrors LoadingIndicatorAnimatorDelegate: 650 ms per shape, spring-driven morph, dual rotation. */
export class LoadingAnimator {
  private spring = new Spring(200, 0.6);
